...
- Process applicability: 10x-deployment's PaaS foundation/config generation is skipped because this repair does not propose new PaaS configuration or a manual Worker deployment. After the human Toolkit merge, existing push/master CI retains its Worker upload → smoke → traffic promotion path; it does not publish course content. Read-only Wrangler preflight and Toolkit Worker build checks supply the relevant evidence. Browser 10x-e2e is not applicable to CLI workflow changes; real CLI/API hosted matrices and actual npm acceptance remain required. No archive until actual completion.
...
## Corrected CLI head and full second proof — 2026-09-13T12:38Z

CLI repair head is now `4883721242fbd1d82e5278901f9b2e984fc7fc91` (Windows npm follow-up); Toolkit stays `1d5c1990df949670c5673f7d8bf0fc9589665071`. Local calculation at this exact head still yields1.21.0 from verified npm1.20.0 baseline.

Toolkit34756860298/1 completedSUCCESS for old CLIa0bcd44 on bothOS; its sanitized receipt artifact is10318145016 (`coordinated-receipt-34756860298-1`, SHA256 `50781fd77a3df7e88c3e082d7305a8ecec865e7db913763530ba9f3f48f35401`). Artifacts remain retained. After terminal confirmation and a fresh12:35:46Z all-run/variable/lease inspection, SessionA changed only Toolkit CLI_CANDIDATE_SHA to4883721, updated12:36:21Z. A full rerun (alljobs, not failed-only) started34756860298/2. Its completed input job independently confirms exactCLI4883721 was snapshotted; no mixing with attempt1.

CLI [CI34757508517/1](https://github.com/przeprogramowani/10x-cli/actions/runs/34757508517) tests4883721: Linux and version-bootstrapPASS; Windows unit tests including strictnpm11.12.1 and bothbuildsPASS, smoke stillinprogress atthisobservation. The private pointers remain old pending complete fresh Toolkit proof. Actual runtime capability/public receipt verification remains pending.

## External evidence gate and independent Windows smoke diagnostic

Toolkit34756860298/2 completedFAILURE during real EN/PL transform: OpenRouterHTTP403, weekly API-key limit exceeded. Snapshot correctly selectedCLI4883721; lint/validationPASS, OS/receiptjobs never ran. No receipt or source-stage substitution is accepted. No provider budget, key scope, entitlement, clock or production policy was changed. The retained-byte release-only workflow is not a PR-mode bypass. A retry needs the operator to restore the existing Toolkit OpenRouter key's weekly allowance, or a separately reviewed/approved plan change for PR retained-byte evidence; no meaningful spending/design choice is inferred. Successful transform logs exposed no aggregate cost, so no invented dollar estimate is supplied.

CLI34757508517/1: Linux+version-bootstrapPASS; Windows unit tests (including pinned npm), node/binarybuildsPASS. Windows smoke passedbinary/version/help/dependencyisolation/startup/coursecontract/no-bump checks, then hung on the feature-version-bump fixture forover7minutes. Root verifiedthiswasanownedpull_request run withonlycheck-windows active and nopublicationjobeligible, then cancelledthatrun toobtainlogs. This is CANCELLED, notgreen. Logs pinpoint the copied auto-version script run byBun; production usesNode. A narrow intended-runtime/bounded-process fixture correction is being prepared without changing assertions, source identity or release behavior. Final Windows gate remains pending.


## Allowance restored by operator; final Windows smoke candidate

Operator explicitly confirmed restoration of the existing OpenRouter key allowance after choosing the unchanged full-CI process. Session A will perform one full Toolkit retry after committing the final Windows smoke correction and rechecking writers/lease/pointers. Another quota failure stops paid retries; no budget/key/provider/model change is authorized. The historical attempt2 failure remains recorded above.

Windows smoke correction local gates: target4PASS, deliberate wrong-version regression RED and unconditional index restore,674unit+10smoke/type/lint/buildsPASS on unchanged full-suite retry (initial existing auth-status timeout recorded). Both independent reviews APPROVED. It uses the actual Node production entry with isolated fixture cwd and bounded processes, preserving all assertions and Windows coverage. Actual final hosted Windows and private exact-pair proof remain pending; final commit/run identities follow below after commit. Runtime automation credentials and all human merge/publication/promotion gates are separate.
