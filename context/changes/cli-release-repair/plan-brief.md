...
Koordynator działa w Toolkit. Własność operacji zapisuje na osobnej referencji Git poza masterem, aktualizowanej bez force; blokada obejmuje testy i publikację w obu repo. Potrzebne są tokeny o wskazanych w planie zakresach, w tym odczyt blokady Toolkit przez zaufany workflow CLI. Koordynator domyślnie pozostaje nieaktywny do weryfikacji konfiguracji przez Session A. Operator doprecyzował podczas implementacji: wyłączenie automatu blokuje nowe wydania, ale proces, który już przejął blokadę, może się zakończyć; nie dodajemy tokena do bieżącego odczytu flagi. Token weryfikatora dowodów otrzymuje zatwierdzony odczyt PR-ów Toolkit, aby sprawdzać ich aktualny SHA i stan; token z odczytem Contents pozostaje dostępny wyłącznie zaufanym zadaniom mastera. Nie dodajemy bazy ani nowej usługi. Opóźnione, powtórzone lub niejednoznaczne zdarzenia nie mogą wywołać drugiego wydania.
