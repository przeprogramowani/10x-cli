...
### PR review fixes and rerun — 2026-09-12

Public CLI initial run `34706552849` passed normal Linux/Windows checks but stopped before any private material download; its artifact API returned zero artifacts. Review identified public paid-content artifacts/logging risk. Fixed by confining all material-bearing work to private Toolkit and exposing only strict sanitized receipts tied to the canonical successful run, exact pair, both platform outcomes and release identity. Receipt regression suites and independent review pass; actual private/public remote handoff remains pending source bootstrap.
